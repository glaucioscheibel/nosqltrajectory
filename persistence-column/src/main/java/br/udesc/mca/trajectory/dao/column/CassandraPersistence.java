...
public class CassandraPersistence extends PersistenceDAO {
    ...
    private Class type; // guarda a referencia prq recupera por reflection eh um
                        // chute
    ...
    public void createDB() {
        /*
         * try { this.db.execute(
         * "CREATE KEYSPACE dba WITH replication = {'class':'SimpleStrategy'," +
         * " 'replication_factor':3};"); this.db.execute(
         * "create table dba.customer (id int primary key, name text);");
         * this.db.execute(
         * "create table dba.customerdata (id int primary key, customerid int, key text, value text);"
         * ); this.db.execute(
         * "create index customerid on dba.customerdata (customerid)"); } catch
         * (Exception e) { this.log.error(e.getMessage(), e); }
         */
    }
    ...
    // ////////////////////////////TEstando metodos abaixo... estao longe de
    // estar na versao final
    @Override
    ...
    /**
     * Cria a tabela baseados nos atributos de uma classe (obrigatorio a classe
     * ter um atributo long id)... atualmente soh persiste primitivos e string
     * 
     * @param
     */
        ...
        if (tb == null) {
            ...
            for (Field f : fs) {
                if (Modifier.isStatic(f.getModifiers())
                        || (!f.getType().isPrimitive() && !String.class.equals(f.getType()))) {// desconsiderar
                                                                                               // staticos
                                                                                               // e
                                                                                               // nao
                                                                                               // primitivos
            ...
            sb.append(")");
            this.db.execute(sb.toString());
            // this.db.execute("create index "+table+"id on "+qname+"(id)"); PK
            // jah eh indice
        }
    }
}
