...
public class MongoLayer {
    ...
    private final String metaMapFunc = "function() { mapfields_recursive (\"\", this);}";
    /** Metadata reduce function (ensure no comments) */
    private final String metaReduceFunc = "function (key, vals) {  sum = 0;  for (var i in vals) sum += vals[i];"
            + "  return sum;}";
    /** Name of collection holding metadata results */
    private final String metaResultsColl = "FieldsAndTypes";
     ...
     */
    static HashMap<String, String> classNameMap = new HashMap<>();
    static {
    ...
    public MongoLayer(DBCollection coll, MongoPluginConfig config) {
        ...
        this.metaData = getCollectionModel(coll, this.buildRule);
    ...
    private DBObject getCollectionModel(DBCollection coll, RecordBuilder buildRule) {
        ...
        coll.mapReduce(this.metaMapFunc, this.metaReduceFunc, this.metaResultsColl, new BasicDBObject());
    ...
    private HashMap<String, ClassCount> getFieldMap(DBCollection metaResultsColl) {
        ...
        HashMap<String, ClassCount> fieldMap = new HashMap<>();
                ...
                if (propMap.get(propKey) != null) {
                    ...
                    if (propMap.get(propKey).equals("com.mongodb.BasicDBObject")) {
                        ...
                        try {
                            ...
                            // arg.
                            try {
                                json.put(propKey, (Class.forName(propMap.get(propKey)).getConstructor(String.class))
                                        .newInstance("0"));
                            } catch (Exception e) {}
                        } catch (Exception e) {}
                    }
        ...
        public String getMajorityClass(String propKey, RecordBuilder buildRule) {
            ...
            if (keys.size() > 1 && buildRule.equals(RecordBuilder.STRING) && !keys.contains("geometry.type")) {
                ...
                for (String currClass : keys) {
                    if (this.classMap.get(currClass) > max) {
