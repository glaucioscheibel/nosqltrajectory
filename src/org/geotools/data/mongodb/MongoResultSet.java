...
public class MongoResultSet {
    ...
    public MongoResultSet(MongoLayer layer, BasicDBObject query) {
        this.layer = layer;
        this.bounds = new ReferencedEnvelope(0, 0, 0, 0, layer.getCRS());
    ...
    private void buildFeatures(BasicDBObject query) {
        ...
        try {
            ...
            mongo = new MongoClient(this.layer.getConfig().getHost(), this.layer.getConfig().getPort());
            ...
            while (cur.hasNext()) {
                ...
                if (!this.layer.getGeometryType().equals(recordGeoType)) {
                    continue;
                }
                ...
                if (recordGeometry != null) {
                    ...
                    this.bounds = new ReferencedEnvelope(this.minX, this.maxX, this.minY, this.maxY,
                            this.layer.getCRS());
                } else {
                    fb.reset();
                }
        ...
        for (String col : cols) {
            ...
            if (dbcol instanceof BasicDBObject || dbcol instanceof BasicBSONList) {
                ...
                // or if bindings subclass Number then cast (possibly lossy)
                        ...
                        || (featureBinding.getSuperclass().equals(Number.class) && dboBinding.getSuperclass().equals(
                                Number.class))) {
                    try {
    ...
    public void paginateFeatures(int startIndex, int maxFeatures) {
        ...
        if (startIndex >= 0 && maxFeatures > 0 && endIndex < this.features.size()) {
            this.features = new ArrayList<>(this.features.subList(startIndex, endIndex));
        }
    }
    ...
    private Polygon createPolygon(BasicDBList polyCoords) {
        Vector<ArrayList<Coordinate>> rings = new Vector<>();
        boolean success = true;
