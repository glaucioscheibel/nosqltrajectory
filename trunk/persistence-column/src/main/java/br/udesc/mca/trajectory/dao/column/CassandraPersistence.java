...
public class CassandraPersistence extends ColumnPersistence {
    ...
    public Trajectory store(Trajectory o) {
        ...
        if (t == null) {
            sb.append("insert into ").append(table)
                    .append("(id,lastModified,description,originalTrajectory,versions) values(?,?,?,?,?)");
            ...
            sb.append("update ").append(table)
                    .append(" set lastModified=?, description=?, originalTrajectory=?, versions=? where id=?");
            PreparedStatement st = this.db.prepare(sb.toString());
            BoundStatement bs = st.bind();
            fillBoundStatement(bs, o);
    ...
    private void fillBoundStatement(BoundStatement bs, Trajectory trajectory) {
        ...
        if (trajectory.getVersions() != null) {
            List<UDTValue> versions = new ArrayList<>();
            UserType versionUT = db.getCluster().getMetadata().getKeyspace("Trajectory").getUserType("version");
            UserType dataUT = db.getCluster().getMetadata().getKeyspace("Trajectory").getUserType("data");
            UserType userUT = db.getCluster().getMetadata().getKeyspace("Trajectory").getUserType("user");
            UserType pointUT = db.getCluster().getMetadata().getKeyspace("Trajectory").getUserType("point");
            UserType segmentUT = db.getCluster().getMetadata().getKeyspace("Trajectory").getUserType("segment");
            UserType processUT = db.getCluster().getMetadata().getKeyspace("Trajectory").getUserType("process");
            ...
            for (TrajectoryVersion tv : trajectory.getVersions()) {
                ...
                if (tv.getData() != null) {
                    List<UDTValue> datas = new ArrayList<>();
                ...
                if (tv.getSegments() != null) {
                    List<UDTValue> segments = new ArrayList<>();
                    for (TrajectorySegment s : tv.getSegments()) {
                        UDTValue segment = segmentUT.newValue();
                        segment.setString("transportationMode",
                                s.getTransportationMode() == null ? null : s.getTransportationMode().name());
                        if (s.getData() != null) {
                            List<UDTValue> datas = new ArrayList<>();
                        ...
                        if (s.getPoints() != null) {
                            List<UDTValue> points = new ArrayList<>();
    ...
    private Trajectory readRow(Row row) throws Exception {
        ...
        if (versions != null) {
            for (UDTValue version : versions) {
                ...
                v.setPreviousVersion(
                        version.getInt("previousVersion") == -1 ? null : version.getInt("previousVersion"));
