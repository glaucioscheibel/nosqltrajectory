...
public class DouglasPeuckerNlogN {
    ...
    public static void douglasPeuckerNlogN(TrackPoint[] V, int i, int j, boolean[] R) { //main
		int splitPos = 0;
		double tmp, dist;
		int k;
		...
		do{
			dist = -1;
			//j = stack[topo]; alterado Alexandre
			j = stack[topo].intValue();
			if (i + 1 < j){
				for (k = i + 1; k < j; k++){
	...
	private static void Stack_Push(int e){ /* push element onto stack */
		//stack[++topo] = e; alterado Alexandre
		stack[++topo] = new Integer(e);
	}
	private static int Stack_Pop(){ 		/* pop element from stack (zero if none) */
		//return stack[topo--]; alterado Alexandre
		return stack[topo--].intValue();
	}
